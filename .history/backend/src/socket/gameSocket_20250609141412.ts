  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('get-room-state', (roomCode: string) => {
      try {
        ...
        if (gameState) {
          console.log(
            `🔍 Getting room state for ${roomCode} - current players:`,
          ...
          if (humanPlayer && humanPlayer.id !== socket.id) {
            console.log(
              `🔄 Need to update player ${humanPlayer.nickname} socket ID from ${humanPlayer.id} to ${socket.id}`
            );
          ...
          console.log(
            `📤 Sending room state for ${roomCode} - final players:`,
