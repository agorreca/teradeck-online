  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('get-room-state', (roomCode: string) => {
      try {
        ...
        if (gameState) {
          console.log(`🔍 Getting room state for ${roomCode} - current players:`, 
            gameState.players.map(p => ({ id: p.id, nickname: p.nickname, isAI: p.isAI })));
          
          // For AI games, update the human player's socket ID if needed
          const humanPlayer = gameState.players.find(p => !p.isAI);
          if (humanPlayer && humanPlayer.id !== socket.id) {
            console.log(`🔄 Need to update player ${humanPlayer.nickname} socket ID from ${humanPlayer.id} to ${socket.id}`);
            gameManager.updatePlayerSocketId(
              roomCode,
              humanPlayer.id,
              socket.id
            );
            ...
            socket.join(roomCode);
            console.log(
              `✅ Updated player ${humanPlayer.nickname} socket ID from ${humanPlayer.id} to ${socket.id}`
            );
          }

          const updatedGameState = gameManager.getRoomState(roomCode);
          const finalState = updatedGameState || gameState;
          console.log(`📤 Sending room state for ${roomCode} - final players:`, 
            finalState.players.map(p => ({ id: p.id, nickname: p.nickname, isAI: p.isAI })));
