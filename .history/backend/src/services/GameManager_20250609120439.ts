...
import { createTeraDeckDeck } from '@shared/constants/cards';
...
export class GameManager {
  ...
  private createDeck(): any[] {
    ...
    // Create a basic deck with some sample cards for testing
    const colors = ['backend', 'frontend', 'mobile', 'dataScience'];
    const moduleNames = {
      backend: ['API Gateway', 'Database', 'Auth Service', 'Cache Layer'],
      frontend: ['React App', 'Vue Component', 'Angular Module', 'UI Library'],
      mobile: ['iOS App', 'Android App', 'React Native', 'Flutter Widget'],
      dataScience: [
        'ML Model',
        'Data Pipeline',
        'Analytics Dashboard',
    ...
    // Add module cards (2 of each color)
    colors.forEach(color => {
      moduleNames[color as keyof typeof moduleNames].forEach(name => {
        deck.push({
          id: `module-${color}-${name.replace(/\s+/g, '').toLowerCase()}`,
          type: 'module',
          color,
          name: { es: name, en: name },
          description: { es: `Módulo ${color}`, en: `${color} module` },
          cost: 1,
        });
      });
    });
    ...
    for (let i = 0; i < 20; i++) {
      deck.push({
        id: `bug-${i}`,
        type: 'bug',
        name: { es: `Bug #${i + 1}`, en: `Bug #${i + 1}` },
        description: {
          es: 'Causa problemas en un módulo',
          en: 'Causes problems in a module',
        },
      });
    }

    // Add some patch cards
    for (let i = 0; i < 15; i++) {
      deck.push({
        id: `patch-${i}`,
        type: 'patch',
        name: { es: `Parche #${i + 1}`, en: `Patch #${i + 1}` },
        description: {
          es: 'Protege un módulo de bugs',
          en: 'Protects a module from bugs',
        },
      });
    }
