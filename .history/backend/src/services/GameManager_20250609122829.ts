...
export class GameManager {
  ...
  private processPlayCardAction(gameState: GameState, action: GameAction): void {
    ...
    switch (card.type) {
      case CardType.MODULE:
        // Add module to player's modules
        const moduleCard = card as ModuleCard;
        // Check if player already has this color
        const hasColor = player.modules.some(m => m.color === moduleCard.color && moduleCard.color !== 'multicolor');
        if (hasColor) {
      ...
      case CardType.OPERATION:
        // TODO: Implement bug, patch, and operation logic
        console.log(`TODO: Implement ${card.type} logic`);
        break;
    }
