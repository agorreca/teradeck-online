...
export class GameManager {
  ...
  updatePlayerSocketId(roomCode: string, oldSocketId: string, newSocketId: string): void {
    const room = this.rooms.get(roomCode);
    if (!room) return;
    ...
    if (player) {
      ...
      console.log(`🔄 Player socket ID updated in room ${roomCode}: ${oldSocketId} -> ${newSocketId}`);
    }
  }
}
