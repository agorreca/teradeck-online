...
export class GameManager {
  ...
  handleDisconnection(roomCode: string, playerId: string): void {
    ...
    if (room.gameState.status === ('WAITING' as any) && !(hasAIPlayers && isHost)) {
      this.leaveRoom(roomCode, playerId);
    }
    
    console.log(`🔌 Player ${playerId} disconnected from room ${roomCode}. AI game: ${hasAIPlayers}, Is host: ${isHost}, Removed: ${!(hasAIPlayers && isHost)}`);
  }
