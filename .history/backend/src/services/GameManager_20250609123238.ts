...
export class GameManager {
  ...
  private playModule(gameState: GameState, player: Player, moduleCard: ModuleCard): void {
  ...
  private playBug(gameState: GameState, player: Player, bugCard: Card, actionData: PlayCardActionData): void {
    ...
    const targetModule = targetPlayer.modules.find(m => m.id === actionData.targetModuleId);
    if (!targetModule) {
  ...
  private playPatch(gameState: GameState, player: Player, patchCard: Card, actionData: PlayCardActionData): void {
    if (!actionData.targetModuleId) {
      throw new Error('Patch card requires target module');
    }

    const targetModule = player.modules.find(m => m.id === actionData.targetModuleId);
    if (!targetModule) {
      throw new Error('Target module not found');
    }
  ...
  private playOperation(gameState: GameState, player: Player, operationCard: Card, actionData: PlayCardActionData): void {
