...
export class GameManager {
  ...
  private createDeck(): any[] {
    ...
    console.log(`📊 Deck composition:
      - Modules: ${deck.filter(c => c.type === 'module').length}
      - Bugs: ${deck.filter(c => c.type === 'bug').length} 
      - Patches: ${deck.filter(c => c.type === 'patch').length}
      - Operations: ${deck.filter(c => c.type === 'operation').length}`);
    return deck;
  }
