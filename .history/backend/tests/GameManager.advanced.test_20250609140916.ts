...
describe('GameManager - TeraDeck Advanced Rules', () => {
  ...
  describe('Win Conditions', () => {
    ...
    test('should NOT win with bugged modules', () => {
      ...
      const stableCount = modulesWithBug.filter((m: any) => m.bugs.length === 0).length;
    ...
    test('should win with multicolor module counting as 4th', () => {
      ...
      const stableCount = modulesWithMulticolor.filter((m: any) => m.bugs.length === 0).length;
      expect(stableCount).toBe(4);
      expect(hostPlayer.modules).toHaveLength(4);
    });
  });
    ...
    test('should allow stealing non-stabilized modules', () => {
      ...
      const stealableModules = [
        ...
        },
        {
          ...
          state: ModuleState.PATCHED,
          bugs: [],
          patches: [{ id: 'patch-1', type: CardType.PATCH, color: ModuleColor.MOBILE }],
          isStabilized: false,
        },
      ] as any[];
  ...
  describe('Phishing Interno (Bug Transfer)', () => {
    test('should only allow bug transfer to FREE modules', () => {
      ...
      const invalidTargets = [
        ...
        },
        {
          ...
          bugs: [{ id: 'bug-2', type: CardType.BUG, color: ModuleColor.MOBILE }],
          patches: [],
          isStabilized: false,
        },
        {
          ...
          bugs: [],
          patches: [
            { id: 'patch-2', type: CardType.PATCH, color: ModuleColor.DATA_SCIENCE },
            { id: 'patch-3', type: CardType.PATCH, color: ModuleColor.DATA_SCIENCE },
          ],
          isStabilized: true,
        },
      ] as any[];
    ...
    test('should keep bugs that cannot be transferred', () => {
      ...
      const hostBuggedModule = {
        ...
        color: ModuleColor.BACKEND,
        state: ModuleState.BUGGED,
        bugs: [{ id: 'bug-backend-1', type: CardType.BUG, color: ModuleColor.BACKEND }],
        patches: [],
        isStabilized: false,
      } as any;
  ...
  describe('Color Compatibility Rules', () => {
    test('multicolor compatibility should work correctly', () => {
      ...
      const backendBug = { id: 'bug-1', type: CardType.BUG, color: ModuleColor.BACKEND };
      const frontendPatch = { id: 'patch-1', type: CardType.PATCH, color: ModuleColor.FRONTEND };
    ...
    test('multicolor bugs/patches should work on any module', () => {
      ...
      const multicolorBug = { id: 'bug-1', type: CardType.BUG, color: ModuleColor.MULTICOLOR };
      const multicolorPatch = { id: 'patch-1', type: CardType.PATCH, color: ModuleColor.MULTICOLOR };
