...
// Create a version of App without the Router wrapper for testing
const AppWithoutRouter = () => {
  const { Routes, Route, Navigate } = require('react-router-dom');
  const { GameBoard } = require('../components/game/GameBoard');
  const { GameRoom } = require('../components/game/GameRoom');
  const { Lobby } = require('../components/lobby/Lobby');
