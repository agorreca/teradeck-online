...
import { TargetType, CardType } from '../../../../shared/src/types/game';
...
describe('useTargeting Hook', () => {
  ...
  describe('startTargeting', () => {
    ...
    it('should call utility functions with correct parameters', () => {
      ...
      expect(mockTargetingUtils.getValidTargets).toHaveBeenCalledWith(card, gameState, 'player1');
      expect(mockTargetingUtils.getCardTargetRequirements).toHaveBeenCalledWith(card);
    });
  ...
  describe('selectTarget', () => {
    beforeEach(() => {
      // Setup targeting state before each test
      const { result } = renderHook(() => useTargeting());
      const card = createMockCard();
      const gameState = createMockGameState();
  ...
  describe('canConfirm', () => {
    it('should return true when target count meets requirements', () => {
      const { result } = renderHook(() => useTargeting());
      const card = createMockCard();
      const gameState = createMockGameState();
    ...
    it('should return false when target count is below minimum', () => {
      mockTargetingUtils.getCardTargetRequirements.mockReturnValue({
        minTargets: 2,
        maxTargets: 2,
        requiredTargets: [TargetType.ENEMY_MODULE],
  ...
  describe('targetingInstructions', () => {
    it('should return instructions from card requirements', () => {
      mockTargetingUtils.getCardTargetRequirements.mockReturnValue({
        minTargets: 1,
        maxTargets: 1,
        description: { es: 'Instrucciones en español', en: 'Instructions in English' },
      });
      ...
      expect(result.current.targetingInstructions).toBe('Instrucciones en español');
    });
