...
describe('TargetingOverlay Component', () => {
  ...
  it('should render when targeting is active', () => {
    const props = {
      targetingState: {
        isActive: true,
        cardBeingPlayed: { id: 'card1', type: 'bug' },
        validTargets: [],
        targetType: 'ENEMY_MODULE',
        requiresTarget: true,
      },
      gameState: { language: 'es' } as any,
      currentPlayerId: 'player1',
      selectedTargets: [],
    ...
    // Mock the translation hook and targeting utils
    jest.doMock('../../hooks/useTranslation', () => ({
      useTranslation: () => ({
        t: (key: string) =>
          key === 'targeting.selectTarget' ? 'Seleccionar objetivo' : key,
      }),
    }));

    jest.doMock('../../../../shared/src/utils/targeting', () => ({
      getCardTargetRequirements: () => ({ maxTargets: 1, minTargets: 1 }),
    }));

    render(<TargetingOverlay {...props} />);

    // Since the component might have complex rendering logic, we just check it doesn't crash
