...
import { GameState, CardType, ModuleCard, Player, Card } from '../../../../shared/src/types/game';
...
describe('GameBoard Component', () => {
  ...
  describe('Loading State', () => {
    it('should show loading screen when no game state', () => {
      ...
      expect(screen.getByText('Conectado - Obteniendo datos del juego...')).toBeInTheDocument();
    });
  ...
  describe('Socket Connection', () => {
    ...
    it('should clean up socket listeners on unmount', () => {
      ...
      expect(mockSocket.off).toHaveBeenCalledWith('game-updated', expect.any(Function));
  ...
  describe('Game State Display', () => {
    ...
    it('should display turn number and current player', () => {
      ...
      expect(screen.getByText('Jugador actual: TestPlayer')).toBeInTheDocument();
    });
  });
  ...
  describe('Card Interaction', () => {
    it('should handle card play when it is player turn', () => {
      const { cardRequiresTarget } = require('../../../../shared/src/utils/targeting');
    ...
    it('should not allow card play when not player turn', () => {
      ...
      expect(mockSocket.emit).not.toHaveBeenCalledWith('game-action', expect.any(Object));
    });
  });
  ...
  describe('Targeting System', () => {
    ...
    it('should handle targeting confirmation', () => {
      ...
      expect(mockSocket.emit).toHaveBeenCalledWith('game-action', expect.objectContaining({
  ...
  describe('Module States', () => {
    ...
    it('should display module with patches', () => {
      const moduleWithPatches: ModuleCard = {
        ...
        description: { es: 'Un módulo con parches', en: 'A module with patches' },
        type: CardType.MODULE,
        color: 'frontend',
        state: 'active',
        bugs: [],
  ...
  describe('Helper Functions', () => {
    it('should apply correct CSS classes for module states', () => {
      ...
      const moduleElement = screen.getByText('Backend API').closest('.card-module');
    ...
    it('should apply correct CSS classes for card types', () => {
      ...
      const operationCard = screen.getByText('Carta de Prueba').closest('.card');
