...
const mockTargeting = {
  ...
  confirmTargeting: jest.fn().mockReturnValue(true),
  cancelTargeting: jest.fn(),
  isTargetSelected: jest.fn().mockReturnValue(false),
  unselectTarget: jest.fn(),
  ...
  useTranslation: () => ({
    t: (key: string) => {
      const translations: Record<string, string> = {
        ...
        'ui.game.turno': 'Turno',
        'ui.game.drawPile': 'Mazo',
        'ui.game.discardPile': 'Pila de Descarte',
        'ui.game.yourModules': 'Tus Módulos',
        'ui.game.yourHand': 'Cartas en mano',
        'ui.game.otherPlayers': 'Otros Jugadores',
        'ui.game.empty': 'Vacía',
        'ui.game.cards': 'cartas',
        'ui.game.card': 'carta',
      };
...
jest.mock('./TargetingOverlay', () => ({
  TargetingOverlay: ({ targetingState }: any) =>
    targetingState.isActive ? (
      <div data-testid="targeting-overlay">Targeting Overlay</div>
    ) : null,
}));

const createGameState = (overrides = {}): GameState => ({
  gameId: 'test-game',
  roomCode: 'TEST123',
  players: [
    {
    ...
    },
    {
      ...
      hand: [{ id: 'card2' } as any],
      modules: [],
    },
  ],
  deck: [{ id: 'deck-card' } as any],
  discardPile: [
    {
      id: 'discard1',
      type: 'operation',
...
const renderGameBoard = (gameState?: GameState) => {
  ...
  // Simulate socket connection and data reception
  if (gameState) {
    // Simulate the socket events that GameBoard listens to
    ...
    // Simulate receiving room state data
    if (socketHandlers['room-state']) {
      socketHandlers['room-state'](gameState);
    }
  }
