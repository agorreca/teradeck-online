...
jest.mock('@shared/constants/cards', () => ({
  getCardName: jest.fn((card, language) => {
    ...
    return card.name[language] || card.name.es || card.name.en || 'Test Card';
  }),
  getCardDescription: jest.fn((card, language) => {
    if (!card || !card.description) return 'Test Description';
    return card.description[language] || card.description.es || card.description.en || 'Test Description';
  }),
  getCardTypeLabel: jest.fn((type, language) => {
    ...
    return labels[type]?.[language] || labels[type]?.es || type;
  }),
  getCardColorName: jest.fn((color, language) => {
    ...
    return colorNames[color]?.[language] || colorNames[color]?.es || color;
  }),
}));
