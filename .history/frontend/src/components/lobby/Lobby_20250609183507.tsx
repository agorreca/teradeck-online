import { AIDifficulty, Language } from '@shared/types/game';
...
export function Lobby() {
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      const settings = {
        maxPlayers: 6, // Default max players for regular games
        aiPlayers: 0,
        language,
                ...
                <div className="ai-options mt-3 p-4 bg-gray-50 rounded-lg border-2 border-gray-200 space-y-4">
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="font-semibold text-gray-700 text-sm">
                      {isSpanish
                        ? 'Número de oponentes (1-5):'
                        : 'Number of opponents (1-5):'}
                    </label>
                  ...
                  >
                    {isCreating ? (
                      ...
                      `🚀 ${isSpanish ? 'Empezar Partida' : 'Start Game'} (${aiPlayers} ${isSpanish ? 'oponentes' : 'opponents'})`
                    )}
                  </button>
                </div>
              )}
