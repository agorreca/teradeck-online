...
export function GameRoom() {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      ...
      console.log(
        '🔍 Socket connected, requesting room state for:',
        roomCode,
        'with socket ID:',
        socket.id
      );
    ...
    if (socket.connected) {
      setCurrentPlayerId(socket.id);
      console.log(
        '🔍 Socket already connected, requesting room state for:',
        roomCode,
        'with socket ID:',
        socket.id
      );
    ...
    socket.on('room-state', (state: GameState) => {
      console.log('📥 Received room-state:', {
        players: state.players.map(p => ({ id: p.id, nickname: p.nickname, isAI: p.isAI })),
