          ...
          {cards.map(card => {
            ...
            return (
              <button
                ...
                `}
                onClick={() => (isSelected || canSelect) && handleCardToggle(card)}
              ...
              >
                ...
                <div className="card-footer mt-2">
                  {card.color && (
                    <div className={`card-color-indicator w-4 h-4 rounded-full ${getModuleColorClass(card.color)}`} />
                  )}
                  {isSelected && (
...
function getModuleColorClass(color: string): string {
  const colorMap: { [key: string]: string } = {
    ...
    multicolor: 'bg-gradient-to-r from-blue-500 via-yellow-500 via-red-500 to-green-500',
  };
