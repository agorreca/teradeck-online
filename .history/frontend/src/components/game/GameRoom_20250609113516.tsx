...
export function GameRoom() {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      ...
      console.log('🔍 Socket connected, requesting room state for:', roomCode, 'with socket ID:', socket.id);
    ...
    if (socket.connected) {
      setCurrentPlayerId(socket.id);
      console.log('🔍 Socket already connected, requesting room state for:', roomCode, 'with socket ID:', socket.id);
      socket.emit('get-room-state', roomCode);
    }
  ...
  const currentPlayer =
    gameState.players.find(p => p.id === currentPlayerId) || humanPlayers[0]; // Fallback to first human player

  // For AI games, if there's only one human player, they should be the host
  const isAIGame = aiPlayers.length > 0;
  const isHost =
    currentPlayer?.isHost || (isAIGame && humanPlayers.length === 1);
  ...
  console.log('Debug GameRoom:', {
    ...
    gameStatePlayersDetailed: gameState.players.map(p => ({
      id: p.id,
      nickname: p.nickname,
      isAI: p.isAI,
    })),
    currentPlayer,
    currentPlayerSearch: gameState.players.find(p => p.id === currentPlayerId),
    isHost,
    isAIGame,
    humanPlayers,
    humanPlayersDetailed: gameState.players
      .filter(p => !p.isAI)
