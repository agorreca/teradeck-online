...
export function getCardTargetRequirements(card: Card): CardTargetRequirements {
  ...
  if (card.type === CardType.OPERATION && card.effect) {
    key =
      `${card.type}_${card.effect}` as keyof typeof CARD_TARGET_REQUIREMENTS;
  }

  return (
    CARD_TARGET_REQUIREMENTS[key as keyof typeof CARD_TARGET_REQUIREMENTS] ||
    CARD_TARGET_REQUIREMENTS[card.type]
  );
}
